...
@trip_tools_bp.route("/api/trip/<int:trip_id>/summary", methods=["GET"])
@jwt_required()
def get_trip_summary(trip_id: int):
    """
    Post-trip summary with planned vs actual spend, highlights, and review prompt.

    Returns a cached summary if available (generated by the daily post_trip task),
    or computes it on the fly if the trip has ended and no cache entry exists.
    """
    user_id = int(get_jwt_identity())
    ...
    try:
        ...
        cached = get_metric(f"post_trip_summary:{trip_id}", parse_json=True)
        if cached:
    ...
    try:
        ...
        summary = generate_trip_summary(trip, db.session)

        # Cache for 30 days
        try:
            from backend.services.metrics_service import set_metric
            set_metric(f"post_trip_summary:{trip_id}", summary, ttl_seconds=30 * 24 * 60 * 60)
        except Exception:
            pass
        ...
        log.exception("trip_tools.summary_failed")
        return jsonify({"error": "Internal server error", "request_id": getattr(g, "request_id", None)}), 500
